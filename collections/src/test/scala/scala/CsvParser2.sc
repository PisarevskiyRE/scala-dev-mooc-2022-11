...
object TestExecution{
  ...
  def main(): Unit = {
    ...
    def IntField =  StringField.map(_.toInt) //StringField.map(...здесь применить parse который подхватит нужный given автоматически ...)
    def FloatField = StringField.map(_.toFloat)
    // def BooleanField StringField.map(_)
